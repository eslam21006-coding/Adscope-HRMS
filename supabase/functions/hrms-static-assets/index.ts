...
const MONTHLY_HOURS_PATCH_MARKER = '<!-- data-adscope-monthly-hours="owner-overtime-v1" -->'
...
(function(){
  ...
  function initializationPending(){return !settled;}
        ...
        auth.onAuthStateChange=function(callback){
          return onAuthStateChange(function(event,session){
            window.setTimeout(function(){
              Promise.resolve().then(function(){return callback(event,session);}).catch(function(){if(initializationPending())showInitializationError();});
            },0);
          });
        };
  ...
  watchdog=window.setTimeout(function(){if(initializationPending())showInitializationError();},timeoutMs);
})();
</script>`
...
(function(){
  ...
  function friendlyMessage(raw){
    ...
    var rules=[
      ...
      [/Only an Owner can approve or reject overtime/i,'Only an Owner can approve or reject overtime. The attendance record was not changed.'],
  ...
  function showEnglishError(message,field){
    ...
    if(root===document.body){node.style.cssText='position:fixed;left:16px;right:16px;bottom:18px;z-index:2147483647;padding:14px 16px;border:1px solid #b94a55;border-radius:12px;background:#23151b;color:#fff;font:14px/1.45 Arial,sans-serif;box-shadow:0 14px 40px #0008';}
  ...
  function reenableVisibleSubmitButtons(form){
    ...
    form.querySelectorAll('button[type="submit"][disabled]').forEach(function(button){button.disabled=false;button.removeAttribute('aria-busy');});
  }
  function normalizeNode(node){
    ...
    candidates.forEach(function(item){
      ...
      if(friendly!==original){item.textContent=friendly;item.setAttribute('data-friendly','1');}
    });
  }
  document.addEventListener('submit',function(event){
    ...
    var problems=[];
    var firstField=null;
    if(currency&&!String(currency.value||'').trim()){problems.push('Select a payroll currency.');firstField=firstField||currency;}
    var compensationValue=compensation?String(compensation.value||''):'unknown';
    var salaryValue=salary?Number(salary.value||0):null;
    if(compensation&&compensationValue!=='commission_only'&&salary&&salaryValue<=0){problems.push('Enter a monthly base salary greater than 0, or change Compensation type to Commission only.');firstField=firstField||salary;}
    var attendanceRequired=false;
    if(attendance){attendanceRequired=attendance.type==='checkbox'?Boolean(attendance.checked):['true','required','yes','1','on'].indexOf(String(attendance.value||'').toLowerCase())>=0;}
    if(attendanceRequired&&shift&&!String(shift.value||'').trim()){problems.push('Assign a work shift because attendance is marked as Required.');firstField=firstField||shift;}
    if(problems.length){event.preventDefault();event.stopImmediatePropagation();showEnglishError(problems.join(' '),firstField);reenableVisibleSubmitButtons(form);}
  },true);
  var uxObserver=new MutationObserver(function(mutations){mutations.forEach(function(mutation){mutation.addedNodes.forEach(function(node){normalizeNode(node);});});});
...
const MONTHLY_HOURS_PATCH = String.raw`<script data-adscope-monthly-hours="owner-overtime-v1">
(function(){
  'use strict';
  if(!window.supabase||!window.ADSCOPE_CONFIG)return;
  var cfg=window.ADSCOPE_CONFIG;
  var hoursClient=window.supabase.createClient(cfg.supabaseUrl,cfg.supabasePublishableKey,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:false}});
  var actorRole=null;
  ...
  function hours(minutes){
    var value=Math.abs(Number(minutes||0));
  ...
  function signed(minutes){
    ...
    return (value>0?'+':value<0?'−':'')+hours(value);
  }
  async function loadActor(){
    if(actorLoaded)return actorRole;
    var sessionResult=await hoursClient.auth.getSession();
    var session=sessionResult.data&&sessionResult.data.session;
    if(!session){actorLoaded=true;return null;}
    var membershipResult=await hoursClient.from('organization_memberships').select('role').eq('user_id',session.user.id).eq('is_active',true).limit(1).maybeSingle();
    if(membershipResult.error)throw membershipResult.error;
    actorRole=membershipResult.data&&membershipResult.data.role||null;
    actorLoaded=true;
    return actorRole;
  }
  function attendanceOpen(){
    var heading=document.querySelector('.topbar h1');
    return Boolean(heading&&heading.textContent.trim()==='Attendance'&&document.getElementById('content'));
  }
  function protectOvertimeModal(){
    var select=document.querySelector('.modal select[name="overtime_approval"]');
    if(!select||select.dataset.ownerProtected==='1'||!actorLoaded)return;
    select.dataset.ownerProtected='1';
    ...
    if(actorRole!=='owner'){
      ...
      if(label)label.textContent='Overtime approval — Owner only';
      var hidden=document.createElement('input');
      hidden.type='hidden';hidden.name='overtime_approval';hidden.value=value;
      if(field)field.appendChild(hidden);
    }else{
      if(label)label.textContent='Overtime approval — does not create payment';
      var form=select.closest('form');
      if(form&&!form.querySelector('[data-overtime-policy-note]')){
        var note=document.createElement('div');
        note.className='wide notice';note.dataset.overtimePolicyNote='1';
        note.innerHTML='<strong>Overtime and payroll are separate</strong><p>Approving this record confirms the overtime hours only. It does not add money to payroll. Any payment requires a separate Owner-approved payroll adjustment.</p>';
        var row=form.querySelector('.form-grid');
        if(row)row.insertBefore(note,row.querySelector('.button-row'));
      }
    }
  }
  function summaryPanel(){
    ...
    var panel=content.querySelector('[data-monthly-hours-summary]');
    if(panel)return panel;
    panel=document.createElement('section');
    panel.className='panel';
    panel.dataset.monthlyHoursSummary='1';
    panel.innerHTML='<header class="panel-head"><div><h2>Monthly hours balance</h2><p class="muted">Worked hours are balanced across the month. Extra hours do not create payroll money automatically.</p></div></header><div class="panel-body"><div class="empty"><div class="spinner"></div>Loading monthly hours…</div></div>';
    content.prepend(panel);
    return panel;
  }
  async function renderSummary(force){
    if(!attendanceOpen()||pending)return;
    var monthInput=document.getElementById('activeMonth');
    var month=monthInput&&monthInput.value;
    if(!/^\d{4}-\d{2}$/.test(String(month||'')))return;
    var panel=summaryPanel();
    var key=month+'|'+(panel&&panel.isConnected?'1':'0');
    if(!force&&key===lastKey&&panel&&panel.dataset.loaded==='1')return;
    pending=true;
    try{
      await loadActor();
      protectOvertimeModal();
      var result=await hoursClient.rpc('get_monthly_hours_summary',{p_month_start:month+'-01'});
      if(result.error)throw result.error;
      var rows=Array.isArray(result.data)?result.data:[];
      var body=panel.querySelector('.panel-body');
      var notice='<div class="notice"><strong>Owner approval required</strong><p>Only an Owner can approve or reject overtime. Approved overtime remains an hours record and is not converted into salary automatically.</p></div>';
      var table='<div class="table-wrap"><table class="data-table"><thead><tr><th>Employee</th><th>Required to date</th><th>Worked</th><th>Balance</th><th>Recorded overtime</th><th>Owner-approved overtime</th></tr></thead><tbody>';
      table+=rows.map(function(row){
        var balance=Number(row.balance_minutes||0);
        var style=balance>0?'color:#268b54;font-weight:800':balance<0?'color:#c74b50;font-weight:800':'font-weight:800';
        return '<tr><td><strong>'+html(row.employee_name||'—')+'</strong><br><span class="code">'+html(row.employee_code||'')+'</span></td><td>'+hours(row.required_minutes)+'</td><td>'+hours(row.worked_minutes)+'</td><td style="'+style+'">'+signed(balance)+'</td><td>'+hours(row.recorded_overtime_minutes)+'</td><td>'+hours(row.approved_overtime_minutes)+'</td></tr>';
      }).join('');
      table+=(rows.length?'':'<tr><td colspan="6">No attendance-required employees were found for this month.</td></tr>')+'</tbody></table></div>';
      body.innerHTML=notice+table;
      panel.dataset.loaded='1';
      lastKey=key;
    }catch(error){
      var body=panel&&panel.querySelector('.panel-body');
      if(body)body.innerHTML='<div class="notice error"><strong>Monthly hours could not be loaded.</strong><p>'+html(window.ADSCOPE_FRIENDLY_ERROR?window.ADSCOPE_FRIENDLY_ERROR(error&&error.message||error):error&&error.message||error)+'</p></div>';
    }finally{pending=false;}
  }
  function inspect(){
    loadActor().then(function(){protectOvertimeModal();renderSummary(false);}).catch(function(){});
  }
  var observer=new MutationObserver(function(){window.clearTimeout(observer._timer);observer._timer=window.setTimeout(inspect,40);});
  observer.observe(document.documentElement,{childList:true,subtree:true});
  document.addEventListener('change',function(event){if(event.target&&event.target.id==='activeMonth'){lastKey='';window.setTimeout(function(){renderSummary(true);},80);}});
  window.setTimeout(inspect,0);
})();
</script>`

function injectUxPatch(html) {
...
function fixEmployeeSaveOrder(html, bundle) {
...
function fixFunctionErrorBody(html, bundle) {
...
function fixUserAccessManagement(html, bundle) {
...
function injectMonthlyHoursPatch(html, bundle) {
  if (bundle !== 'admin' || html.includes(MONTHLY_HOURS_PATCH_MARKER)) return html
  return /<\/body>/i.test(html) ? html.replace(/<\/body>/i, `${MONTHLY_HOURS_PATCH}${MONTHLY_HOURS_PATCH_MARKER}</body>`) : `${html}${MONTHLY_HOURS_PATCH}${MONTHLY_HOURS_PATCH_MARKER}`
}

function updateEmployeePortalUrls(html) {
  return html.replaceAll('https://attendance.adscope.net/', EMPLOYEE_PORTAL_URL).replaceAll('https://attendance.adscope.net', EMPLOYEE_PORTAL_URL.slice(0, -1))
}

function injectInitializationRecovery(html) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const basePatchedHtml = injectUxPatch(injectInitializationRecovery(updateEmployeePortalUrls(userAccessHtml)))
    const patchedHtml = injectMonthlyHoursPatch(basePatchedHtml, bundle)
    ...
    if (bundle === 'admin' && !patchedHtml.includes(MONTHLY_HOURS_PATCH_MARKER)) throw new Error('Monthly hours patch was not applied')
    ...
    return new Response(payload, {
      headers: {
        ...
        'X-HRMS-Patch': [
          ...
          patchedHtml.includes(MONTHLY_HOURS_PATCH_MARKER) ? 'monthly-hours-owner-overtime-v1' : null,
        ].filter(Boolean).join(','),
      },
    })
  } catch (error) {
    console.error(error)
    return Response.json({ error: 'Unable to load the HRMS portal' }, { status: 500, headers: { ...cors, 'Cache-Control': 'no-store' } })
  }
})
