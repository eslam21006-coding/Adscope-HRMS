-- Keep every newly generated Adscope full-time Saturday attendance row aligned
-- with the company Saturday schedule, regardless of which attendance-generation
...
as $function$
declare
  v_is_adscope_full_time boolean := false;
...
begin
  ...
  select e.employment_type='full_time' and o.code='ADSCOPE'
  into v_is_adscope_full_time
  from public.employees e
  join public.organizations o on o.id=e.organization_id
  where e.id=new.employee_id
    and e.organization_id=new.organization_id;

  if not coalesce(v_is_adscope_full_time,false) then
    return new;
  end if;
...
comment on function app_private.apply_full_time_saturday_schedule_on_insert() is
  'Normalizes newly inserted Adscope full-time Saturday attendance rows with an effective shift to 12:00-21:00 with a 60-minute break from 2026-08-29 onward; other organizations, official holidays and no-shift rows remain unchanged.';
